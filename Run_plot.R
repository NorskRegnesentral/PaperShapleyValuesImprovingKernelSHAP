    ...
    dt_now_tmp <- data.table::rbindlist(
      lapply(repetitions, function(repetition) {
        ...
        file_name <- file.path(
          folder_save,
          paste0(
            paste("M", m, "n_train", n_train, "n_test", n_test, "rho", rho, "equi", rho_equi,
              "betas", paste(as.character(betas), collapse = "_"),
              "MAE_repetition", repetition,
              sep = "_"
            ),
            ".rds"
          )
...
res_MAE <- res[, .(MAE_mean = mean(MAE), MAE_lower = quantile(MAE, 0.025), MAE_upper = quantile(MAE, 0.975)),
...
fig_MAE <-
  ...
  coord_cartesian(ylim = c(10^(-4), 10^0))
fig_MAE


# To illustrate that the width of the confidence bands decrease as N_S increases
fig_MAE2 <- ggplot(
  res_MAE[Strategy %in% c("Unique", "PySHAP* C-Kernel")],
